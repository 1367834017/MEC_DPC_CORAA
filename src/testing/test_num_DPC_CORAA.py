# Performance analysis of DPC-CORAA, including the impact of number of UDs, ESs
...
def main(env):
    ...
    agents = py.MADDPG(env, state_dims, action_dims, critic_dim, 32)

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    agents.load(path='DPC.pth', map_location=device)
    reward_history = []
    for i in range(20):
        states = env.reset()
        reward_t = []
