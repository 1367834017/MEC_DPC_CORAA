...
# ----------------------------------------------------------------------
    def step(self, action):
        ...
        # np.clip(action, 0, 1, out=action)  # 直接裁剪，而不是 `if` 语句
        # action[np.isnan(action)] = 1  # 处理 NaN
        # # 拆分 `action_1`
        ...
        # comm = PC.DecPOSGPartialCommunication(N=self.K, action_dim=2 * self.ser + 1, observation_space=self.K * (2 * self.ser + 1), lambda_step=0.1, k_max=500)
        # next_obs = comm.update_step(t=0, current_actions=action, current_obs=self.state)
        #
        # return next_obs, self.reward_i, self.done, {}
# ----------------------------------------------------------------------
    def reset(self):
