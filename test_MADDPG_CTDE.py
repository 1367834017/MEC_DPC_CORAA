...
def main(env):
    ...
    for i in range(env.K):
        state_dims.append((2 * env.ser + 1))  # 110
        action_dims.append(2 * env.ser + 1)  # 11
    critic_dim = sum(state_dims) + sum(action_dims)
    agents = py.MADDPG(env, state_dims, action_dims, critic_dim, 64, device)
    agents.load(path='MADDPG_CTDE.pth')
    reward_history = []
    for i in range(20):
        ...
        for t in range(50):
            ...
            if t % 10 == 0:
                print("Episode: \t{} Reward: \t{:0.6f}".format(i, np.sum(reward_i)))
