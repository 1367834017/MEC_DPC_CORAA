...
class Env():
    ...
    def step(self, action):
        ...
        # self.state = np.zeros((self.K, self.K *(self.ser + 1)))
        ...
        # m = np.count_nonzero(action, axis=0)  # 每列不为0的个数
        # a = self.pi * 0.001 * self.hi
        # r_1 = (self.B / m[:self.ser]) * 1e6 * np.log2(1 + (a / self.N0))  # 直接矩阵运算
        #
        # # **优化4：批量计算时间 & 能耗**
        ...
        # self.state[:] = action.reshape(1, -1)  # 直接赋值到所有行
        #
        #
